...
/** @_list - 当前系统支持的主题列表, 由接口获取, 文件可放置在 public/_list, 采用异步加载 */
const _list: Ref<Set<string>> = ref(new Set([]));
...
/** @hooks 获取所有主题列表, 只读 */
export const useThemeList = () => readonly(_list);
/** @hooks 初始化 */
(() => {
  // 获取当前系统是否是暗色主题
...
function LOAD_THEME(_: string) {
  /** @如果文件已加载 设置主题 */
  if (_loaded.has(_)) return UPDATE_THEME(_);
  ...
  // link.href = import.meta.env.BASE_URL + "_list/" + _ + "_variable.css";
